...
const sequenceDetails = async (settings, executor = db, { lock = false, date = new Date() } = {}) => {
  const period = getTicketPeriod({ prefix: settings.ticketPrefix, sequenceReset: settings.sequenceReset, date });
...
const serializeResponse = (row, sequence) => ({
  ...rowToSettings(row),
...
const getSettings = async () => {
  const row = await settingsRepository.findSettings();
  ...
  return serializeResponse(row, await sequenceDetails(rowToSettings(row)));
};

const preview = async (input, { date = new Date() } = {}) => {
  const storedRow = await settingsRepository.findSettings();
  if (!storedRow) { const error = new Error("Ticket settings are not configured"); error.statusCode = 503; throw error; }
  const validation = validateTicketSettings({ ...rowToSettings(storedRow), ...input });
  ...
  const details = await sequenceDetails(settings, db, { date });
  ...
  return {
    preview: buildTicketNumber({ settings, sequence: previewSequence, preview: true, date }),
    nextPreview: buildTicketNumber({ settings, sequence: nextSequence, preview: true, date }),
...
const updateSettings = async ({ settings, req, reason }) => {
  ...
  try {
    ...
    const oldSequence = await sequenceDetails(oldSettings, connection, { lock: true });
    ...
    if (changes.length) {
      ...
      const newPeriod = getTicketPeriod({ prefix: settings.ticketPrefix, sequenceReset: settings.sequenceReset });
...
const resetSequence = async ({ newStartingSequence, req, reason }) => {
  ...
  try {
    ...
    const details = await sequenceDetails(settings, connection, { lock: true });
    ...
    const nextTicket = buildTicketNumber({ settings, sequence: nextSequence, preview: true });
    if (await sequenceRepository.ticketExists(nextTicket, connection)) {
    ...
    await connection.commit();
    return { data: await getSettings(), nextTicket };
  } catch (error) {
    await connection.rollback();
    throw error;
  } finally {
...
module.exports = { getHistory, getSettings, preview, resetSequence, updateSettings };
